...
from app.db.models import AccessCode, AuditLog, BalanceLedger, CallbackEvent, Gateway, GatewayConfig, GatewayPackage, Order, PayoutRequest, User
...
def list_gateways(db: Session) -> list[Gateway]:
    return db.query(Gateway).order_by(Gateway.code.asc()).all()


def list_enabled_gateway_codes(db: Session) -> list[str]:
    rows = db.query(Gateway).filter(Gateway.enabled.is_(True)).order_by(Gateway.code.asc()).all()
    return [r.code for r in rows]


def enable_gateway(db: Session, code: str) -> None:
    code = code.strip().lower()
    gw = db.get(Gateway, code)
    if gw:
        gw.enabled = True
    else:
        db.add(Gateway(code=code, enabled=True))
    db.commit()


def disable_gateway(db: Session, code: str) -> None:
    code = code.strip().lower()
    gw = db.get(Gateway, code)
    if gw:
        gw.enabled = False
